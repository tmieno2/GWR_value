...
**Abstract**: Geographically weighted regression (GWR) has been presented as a valuable tool for the estimation of site-specific yield response functions to derive variable rate input recommendations. This study uses Monte Carlo simulations to demonstrate that, when GWR's yield response functional form is mis-specified as quadratic while the true yield-input relationship is quadratic-plateau, using GWR can lead to large overestimation of the economic value of variable rate application of input rates compared to its actual value. We encourage the statistical community to develop tools in software packages providing GWR that allow more flexibility in functional form assumptions.
...
Note that the true $N^*_{j,i}$s are not observable by producers or researchers, and are not used in the profit calculation in this study.


## Trial Design

In each simulated on-farm experiment, the field was partitioned into a 4-block $\times$ 2-block grid (see Figure A1 of Appendix II), in which each block was partitioned into a 6-plot $\times$ 6-plot grid with each of six N rates assigned to exactly one plot in each row and each column of the block (i.e., a Latin square). The Latin square implemented here has a special pattern  [@li2023economic] to ensure that no adjacent N plots were assigned the same N rate, and furthermore, the order of N rates in each row of every block was specified to avoid relatively large changes in N rates (due to the technological limitations of N application equipment commonly witnessed in real-world OFPE). 

In each simulation, six trial N rates were assigned by calculating the 0%, 20%, 40%, 60%, 80%, and 100% quantiles of the simulation's true cell-specific plateau N rates ($\tau_{j,i})$. We then subtract 20 kg ha$^{-1}$ from the 0% quantile and add 20 kg ha$^{-1}$ from the 100% quantile to ensure that the range of trial N rates fully encompass the true plateau points. The means of these trial N rates across simulations were approximately {80, 130, 154, 184, 219, and 270} kg ha$^{-1}$, while the differences between simulations were small and negligible. The spatial map of the trial N rates within the field by the Latin square design was illustrated in Figure A2 of Appendix II.
...
## Economic value of the GWR-based VRA

The true economic value of the GWR models was measured by the per-ha net revenue ("profit") increase resulting from the utilization of the VRA based on the GWR models in comparison to the URA. The true profit of GWR-based VRA (either GWRR or GWRT) was generated by substituting the GWR-estimated site-specific optimal N rates ($\hat{N}^{*GWR}_j$) into the true yield response functions:

$$\Pi^{GWR}=\frac{1}{8640} \sum_{j=1}^{1440} \sum_{i=1}^6 [p \cdot f_{j, i} (\hat{N}^{*GWR}_j)-w \cdot \hat{N}^{*GWR}_j],$$

where $f_{j, i}(\cdot)$ denotes the true yield response function at cell $(j, i)$. Similarly, the true profit of URA was based on the SCAM-estimated uniform optimal N rate (\hat{N}^{*SCAM} ):

$$\Pi^{SCAM} = \frac{1}{8640} \sum_{j=1}^{1440} \sum_{i=1}^6 \big[p \cdot f_{j, i} (\hat{N}^{*SCAM})-w \cdot \hat{N}^{*SCAM}\big].$$

The true economic value of the GWR-based VRA was then calculated as:

$$\Delta^{GWR} = \Pi^{GWR} - \Pi^{SCAM} .$$

In practice, since the true responses $f_{j, i}(\cdot)$ are unknown, the calculation of the economic value of the GWR-based VRA is always relying on the assumption that the GWR-estimated yield response functions are true. In this study, the estimated profits of the GWR-based VRA and SCAM-based URA by using the GWR-estimated responses were:
...
where $\hat{f}^{GWR}_{j, i}(\cdot) = \hat{f}^{GWR}_j(\cdot)$ denotes the GWR-estimated yield response function at cell $(j, i)$ within subplot $j$.

The estimated economic value of the GWR-based VRA was:

$$\hat{\Delta}^{GWR} = \hat{\Pi}^{GWR} - \hat{\Pi}^{SCAM}.$$

Because $\hat{f}^{GWR}_{j,i}(\cdot)$ is mis-specified in functional form and deviating from the true response $f_{j,i}(\cdot)$, the estimated economic value of GWR-based VRA $\hat{\Delta}^{GWR}$ is likely biased from the true economic value $\Delta^{GWR}$. That bias in the estimation was calculated as:

$$bias = \hat{\Delta}^{GWR} - \Delta^{GWR}.$$

# Results and Discussions

## The true economic value of the GWR-based VRA

The distribution of the true economic value of the quadratic GWR-based VRA over the SCAM-based URA, $\Delta^{GWR}$, from the 1,000 simulations, is shown in Figure \@ref(fig:value-vra). Three different price scenarios are examined to represent the historically low (5.44), middle (6.56), and high (7.67) nitrogen-corn price ratio (\$/kg : \$/kg). Historical corn ($p$) and nitrogen fertilizer ($w$) prices from 2002 to 2022 were obtained from the USDA National Agricultural Statistics Service [@NASS2022] and DTN Retail Fertilizer Trends [@DTN2022], and were transformed into monthly data. The nitrogen-corn price ratios were calculated as $w/p$. Further information regarding the collection, calculation, and summary statistics of the price data can be found in the supplementary appendix (Appendix 1: Historical Corn and Nitrogen Prices).

The true economic values of VRA derived by the mis-specified site-specific response models (quadratic GWRR and GWRT) are only moderate over URA derived by a correctly specified uniform response model (SCAM) when the true yield response function is quadratic-plateau. The median of the true economic values ranges from as small as \$0.81 per hectare to a moderate \$7.83 per hectare, depending on price scenarios and models used. In addition, in any price and model scenario, nearly half of the true economic values are negative, meaning a considerable probability that the GWR-based VRA are actually less profitable than the SCAM-based URA.

The GWRR and GWRT models are slightly different in their economic performances. Specifically, GWRR performs better as nitrogen-corn price ratio increases, while the opposite is the case for GWRT. Figure \@ref(fig:bias-est-eonr) provides insights into this phenomenon, showing the distribution of the average ratio of estimated EONR to true EONR. A value lower than 1 means that EONR is underestimated on average. GWRR tends to overestimate EONR at lower price ratios, but the magnitude of overestimation falls as the price ratio rises. The opposite trend is true for GWRT, where at the high price ratio the degree of EONR overestimation is quite significant. Since a high-price ratio means nitrogen is relatively expensive, the economic cost of overestimation of EONR increases with the price ratio. Figure \@ref(fig:true-vs-estimated-optn-gwr-r) presents for a single simulation round the estimated site-specific EONR plotted against the true site-specific EONR. The site-specific EONR estimates are both inaccurate, but each in its way. The range of EONR estimates by GWRR is very wide because the coefficient on $N^2$ is allowed to vary site specifically in GWRR. Since the estimation of the coefficient on $N^2$ is highly inaccurate, the range of estimated EONRs is much wider than the range of true site-specific EONRs. However, GWRT's range of EONR estimates is very narrow because the coefficient on $N^2$ is constrained to be equal across sites, making GWRT fail to capture the EONR's true spatial heterogeneity. 


## Bias in the estimated economic value of the GWR-based VRA

The estimated economic values of the GWR-based VRA over the SCAM-based URA, $\hat{\Delta}^{GWR}$, however, are much larger than the true values. The distribution of the estimated economic values is also shown in Figure \@ref(fig:value-vra), where the median ranges from \$17.15 per hectare to as large as \$45.62 per hectare, surpassing the actual values across all simulation scenarios. Moreover, the estimated economic values are positive in almost all rounds of simulation, suggesting that the GWR-based VRA are almost always more profitable than the SCAM-based URA. But this is significantly erroneous, as nearly half of the time the GWR-based VRA is actually less profitable than SCAM-based URA. These simulation results reveal a substantial overestimation of the GWR-based VRA's economic value when the GWR-estimated site-specific yield response functions are used to substitute the true (but unknown) response functions. That is, in applied research, the quadratic GWR models would tend to create a misleading impression of superior performance in VRA recommendations compared to their actual performance.

The bias in the estimated value, i.e., the discrepancy between the estimated and true economic values $\hat{\Delta}^{GWR} - \Delta^{GWR}$, reaches as large as approximately \$40 per hectare for the GWRR model. When the GWR estimates are considered to define true yield response, the inaccuracy of GWRR generates overestimation of the heterogeneity of yield response within the field, which can lead to significant overestimation of the value of GWRR itself. GWRT exhibits a different pattern of bias. At low price ratios, the GWRT approach displays smaller bias of about \$10 per hectare. This is partially because GWRT tends to understate the heterogeneity of the site-specific yield response functions as was seen in Figure \@ref(fig:true-vs-estimated-optn-gwr-r). As the price ratio rises, bias increases to approximately \$30 per hectare. This is due to the significant overestimation of EONR by GWRT, as illustrated by Figure \@ref(fig:bias-est-eonr). Unlike the GWRR approach, the distribution of the bias by the GWRT approach has a fat tail, indicating that there is a greater chance of encountering large positive bias in the estimated value of VRA over URA.

Figure \@ref(fig:why-bias-many) illustrates how overestimation of the value of VRA can occur. It shows quadratic yield response functions estimated by GWRT and the estimated yields associated with the GWR-based EONR rates and SCAM-based EONR rates for 50 randomly chosen cells in a single round of simulation. Since GWR-based EONRs were obtained assuming the yield response functions estimated by GWR, by construction they out-perform the SCAM-based estimates in every cell. For many cells, the SCAM-based estimated EONR is significantly higher than the actual yield-maximizing N rate, resulting in an estimated yield far lower because of the quadratic functional form assumption. Figure \@ref(fig:why-bias-single) panel (a) illustrates the true quadratic-plateau yield response curve and estimated yield response curve by the mis-specified quadratic GWR for a single cell. The cell's true yield response plateaus at a nitrogen rate of about $`r round(il_data_oe[aunit_id %in% single_aunit, Nk], digits = 0)`$ kg ha$^{-1}$, but the GWR-estimated yield response function suggests that yield should peak at a nitrogen rate of about $`r round(point_data_gwr[type == "GWR", N], digits = 0)`$ kg ha$^{-1}$ and decline thereafter. The true yield differential between recommended N rates by GWR and SCAM is 0, while the estimated yield differential is about `r round(point_data_gwr[type == "GWR", yield] - point_data_gwr[type == "SCAM", yield], digits = 2)` ton ha$^{-1}$, leading to the overestimation of the GWR yield benefit. Panel (b) illustrates the corresponding true and estimated profit response curves. Based on the true profit response, applying the GWR-based recommendation is about \$`r round(true_yield_data[type != "True", diff(profit)], digits = 0)` ha$^{-1}$ less profitable than the SCAM-based recommendation, where this profit differential comes solely from the difference in nitrogen cost since GWR and SCAM yields are equal. However, when calculations are based on the cell's GWR-estimated response, profit from the GWR-based recommendation exceeds that from the SCAM-based recommendation by about \$`r round(- point_data_gwr[, diff(profit)], digits = 0)` ha$^{-1}$, which is much larger than the true value of \$`r round(true_yield_data[type != "True", diff(profit)], digits = 0)` ha$^{-1}$. Beyond the difference in nitrogen cost, the falsely estimated yield boost by GWR contributes to the profit differential, thus significantly overestimating the actual value of GWR-based VRA. 

# Conclusions

Our simulation results indicate that variable rate application (VRA) derived from the mis-specified quadratic GWR models (GWRR and GWRT) only has small economic values over uniform rate application (URA) derived from a correctly specified uniform response model (SCAM) when the true yield response function is quadratic-plateau, such as in the corn-nitrogen relationship. However, there is a notable risk of researchers misconstruing the economic values of the quadratic GWR-based VRA as substantially higher than its true values when substituting estimated site-specific yield response functions for the true response functions. This bias in overestimation could be substantial, reaching approximately $40 per hectare. Practitioners in precision agriculture should exercise caution when employing GWR for site-specific input recommendations

Our results clearly show that it can be problematic to use GWR under the assumption of a quadratic functional form when the true yield-input relationship is quadratic-plateau. Previous studies estimating corn yield responses using GWR had to choose the quadratic functional form because no readily usable statistical software or package could be used to implement quadratic-plateau GWR at the time. The only nonlinear model available at the moment is the linear-plateau GWR by @lambert2022geographically. Unfortunately, the estimation of the economically optimal input rate cannot be adjusted according to price using the linear-plateau model as the economically optimal input rate occurs always at the kink (critical input rate where yield plateau is reached). Given that many yield-input relationships follow quadratic-plateau functional form for many crops, it would be fruitful to develop a GWR method with quadratic-plateau functional form. It should be also possible and interesting to build semi-parametric GWR where the yield response functions are estimated without assuming particular functional forms (like SCAM). It is also noted that the recently developed multi-scale GWR model [@fotheringham2017multiscale; @yu2020inference; @comber2022route] allows the bandwidth to be derived separately for each covariate in the model, which would potentially improve the performances of GWR estimates and may need to be incorporated into future research. Note, however, that multi-scale GWR models are not necessary in our simulations as we have only one covariate ($N$) in our simulations.

Most importantly, whether using GWRT or GWRR, researchers must be cautious about their estimated values of VRA when the model is assumed to be quadratic but the true underlying yield-input response is quadratic-plateau. Our results show that it is likely that they will overestimate their true values and the bias can be misleadingly high with non-negligible probability. Indeed, the bias problem we have raised here is likely to be observed for other statistical methods as well because the bias comes from the assumption that the _estimated_ yield response functions are true. Whatever the underlying statistical methods for yield response functions may be, researchers are likely to overestimate the true value of their methods. In the future, it would be interesting to investigate the bias in other commonly used statistical methods in the literature on site-specific yield response estimation (such as regression with interaction terms of field characteristics, machine learning, and more) by using a similar simulation-based evaluation approach as done in this study. 
...
```{r, fig.id = "value-vra", fig.cap = "The economic value of the quadratic GWR-based VRA over SCAM-based URA. The true values are calculated using the true yield response functions, while the estimated values are calculated using the quadratic GWR-estimated yield response functions. Two GWR models (GWRR and GWRT) are used. The values are calculated at three nitrogen-corn price ratios ($/kg : $/kg) while the corn price is set at $0.28/kg.", out.width = "7in", fig.width = 6.5}
g_value
```
...
```{r fig.id = "why-bias-many", fig.cap = "The cause of significant overestimation of the value of GWR-based VRA", fig.width = 6}
# knitr::include_graphics(here::here("GitControlled/Writing/Figures/g_why_bias_many.png"))
g_why_bias_many
```

```{r fig.id = "why-bias-single", fig.cap = "An illustration of overestimation of the value of GWR-based VRA over SCAM-based URA",fig.width = 6.5, fig.height = 6}
