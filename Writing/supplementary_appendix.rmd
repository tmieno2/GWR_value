...
```{r, fig.id = "price", fig.cap = "Historical monthly variations in corn price, nitrogen fertilizer price, and the corresponding nitrogen-corn price ratio, 2002-2022"}
knitr::include_graphics("price_ratio_history.png")
```

Corn prices were sourced from the USDA National Agricultural Statistics Service [@NASS2022] in monthly frequency from 2002 to 2022. As shown in the top panel of Figure A1, the corn prices fluctuated between \$0.07/kg and \$0.30/kg, with an average of $0.15/kg. 

Nitrogen prices were collected from two sources. The USDA National Agricultural Statistics Service provided yearly nitrogen fertilizer prices for the period 2002 to 2014. The DTN Retail Fertilizer Trends [@DTN2022] provided weekly nitrogen fertilizer prices from 2015 to 2022. To align with the monthly corn price data, the nitrogen price data from both sources were combined and converted into monthly data. Three most commonly used nitrogen fertilizer types were included: Liquid urea-ammonium nitrate with 28 percent nitrogen (UAN28), Liquid urea-ammonium nitrate with 32 percent nitrogen (UAN32), and anhydrous ammonia (ANHYD). All prices were all converted to the per kilogram (kg) of nitrogen basis. As shown in the middle panel of Figure A1, the nitrogen prices ranged from \$0.34/kg to \$2.37/kg, with an average of \$0.98/kg.

The nitrogen-corn price ratios were calculated as the nitrogen fertilizer price ($w$) divided by corn price ($p$), and the results were shown in the bottom panel of Figure A1. This study took the values at the 25th, 50th, and 75th percentiles of the historical monthly price ratios (pooled over the three types of nitrogen fertilizer) from 2002 to 2022. These three values were used to represent the low (5.44), middle (6.56), and high (7.67) price ratio scenarios. For consistency purposes, in the profitability calculations the corn price was selected at approximately the Spring 2022 price of \$0.28/kg. Consequently, the corresponding nitrogen prices for the three price ratio scenarios were approximately 5.44×0.28 = \$1.52/kg, 6.56×0.28 = \$1.84/kg, and 10.35×0.28 = \$2.15/kg. It is important to note that when different corn prices are used, the resulting profits of GWR will vary accordingly, but the variation will be proportional to the result of selected corn price and does not qualitatively alter the final conclusions.
...
Figures A\@ref(fig:value-kernel-exp) through A\@ref(fig:value-kernel-tricube) show the distribution of the true economic value of GWR using four different kernels: Exponential, Boxcar, Bi-square, and Tri-cube.
...
```{r, fig.id = "value-kernel-exp", fig.cap = "The true economic value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Exponential kernel function", fig.height = 3.5}
gg_appendix$g_value[[2]]
```

```{r, fig.id = "value-kernel-boxcar", fig.cap = "The true economic value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Boxcar kernel function", fig.height = 3.5}
gg_appendix$g_value[[3]]
```

```{r, fig.id = "value-kernel-bs", fig.cap = "The true economic value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Bi-square kernel function", fig.height = 3.5}
gg_appendix$g_value[[4]]
```

```{r, fig.id = "value-kernel-tricube", fig.cap = "The true economic value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Tri-cube kernel function", fig.height = 3.5}
...
```{r, fig.id = "bias-kernel-exp", fig.cap = "Bias in the estimation of the value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Exponential kernel function", fig.height = 3.5}
gg_appendix$g_bias[[2]]
```

```{r, fig.id = "bias-kernel-boxcar", fig.cap = "Bias in the estimation of the value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Boxcar kernel function", fig.height = 3.5}
gg_appendix$g_value[[3]]
```

```{r, fig.id = "bias-kernel-bs", fig.cap = "Bias in the estimation of the value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Bi-square kernel function", fig.height = 3.5}
gg_appendix$g_value[[4]]
```

```{r, fig.id = "bias-kernel-tricube", fig.cap = "Bias in the estimation of the value of GWR-based VRA for two GWR models (GWRR and GWRT) and under three nitrogen-corn price ratios (5.44 (low), 6.56 (middle), and 7.67 (high) $ kg^(-1)  / $ kg^(-1)), using Tri-cube kernel function", fig.height = 3.5}
gg_appendix$g_value[[5]]
```
